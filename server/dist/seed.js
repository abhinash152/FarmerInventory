...
async function seedDatabase(cleanExisting = true) {
    if (cleanExisting) {
        console.log('🌱 Cleaning existing data in farmer_inventory_system...');
        await prisma_1.prisma.complaint.deleteMany();
        await prisma_1.prisma.notification.deleteMany();
        await prisma_1.prisma.chatMessage.deleteMany();
        await prisma_1.prisma.orderFeedback.deleteMany();
        await prisma_1.prisma.orderExtension.deleteMany();
        await prisma_1.prisma.sale.deleteMany();
        await prisma_1.prisma.order.deleteMany();
        await prisma_1.prisma.product.deleteMany();
        await prisma_1.prisma.farmer.deleteMany();
        await prisma_1.prisma.customer.deleteMany();
    }
    const hashedPw = await bcryptjs_1.default.hash('password123', 10);
    console.log('👩‍🌾 Seeding Farmers...');
    const farmerGurpreet = await prisma_1.prisma.farmer.create({
        data: {
    ...
    const farmerAnita = await prisma_1.prisma.farmer.create({
        data: {
    ...
    const farmerRamesh = await prisma_1.prisma.farmer.create({
        data: {
    ...
    const customerRahul = await prisma_1.prisma.customer.create({
        data: {
    ...
    const customerPriya = await prisma_1.prisma.customer.create({
        data: {
    ...
    const p1 = await prisma_1.prisma.product.create({
        data: {
    ...
    const p2 = await prisma_1.prisma.product.create({
        data: {
            farmer_id: farmerGurpreet.farmer_id,
    ...
    const p3 = await prisma_1.prisma.product.create({
        data: {
            farmer_id: farmerGurpreet.farmer_id,
    ...
    const p4 = await prisma_1.prisma.product.create({
        data: {
    ...
    const p5 = await prisma_1.prisma.product.create({
        data: {
            farmer_id: farmerAnita.farmer_id,
    ...
    const p6 = await prisma_1.prisma.product.create({
        data: {
    ...
    const order1 = await prisma_1.prisma.order.create({
        data: {
            customer_id: customerRahul.customer_id,
            farmer_id: farmerGurpreet.farmer_id,
            product_id: p1.product_id,
            quantity: 10,
            status: prisma_1.OrderStatus.ACCEPTED,
            requested_at: new Date(Date.now() - 3 * 24 * 3600 * 1000),
            responded_at: new Date(Date.now() - 3 * 24 * 3600 * 1000 + 3600 * 1000),
            extension: {
                create: {
                    payment_method: prisma_1.PaymentMethod.ONLINE,
                    payment_status: prisma_1.PaymentStatus.COMPLETED,
                    delivery_pincode: '110016',
                    delivery_fee: 50.0,
                    delivery_address: 'Flat 402, Green Park Avenue, New Delhi',
                    tracking_step: prisma_1.TrackingStep.DELIVERED,
    ...
    // Review for Order 1
    await prisma_1.prisma.orderFeedback.create({
        data: {
    ...
    await prisma_1.prisma.order.create({
        data: {
            ...
            extension: {
                create: {
                    payment_method: prisma_1.PaymentMethod.COD,
                    payment_status: prisma_1.PaymentStatus.PENDING,
                    delivery_pincode: '110016',
                    delivery_fee: 50.0,
                    delivery_address: 'Flat 402, Green Park Avenue, New Delhi',
                    tracking_step: prisma_1.TrackingStep.PLACED,
    ...
    await prisma_1.prisma.chatMessage.create({
        data: {
            farmer_id: farmerGurpreet.farmer_id,
            customer_id: customerRahul.customer_id,
            sender_role: prisma_1.UserRole.CUSTOMER,
    ...
    await prisma_1.prisma.chatMessage.create({
        data: {
            farmer_id: farmerGurpreet.farmer_id,
            customer_id: customerRahul.customer_id,
            sender_role: prisma_1.UserRole.FARMER,
