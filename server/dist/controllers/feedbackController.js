...
exports.getFarmerFeedbacks = exports.submitFeedback = void 0;
...
const submitFeedback = async (req, res) => {
    try {
        ...
        if (order.status !== prisma_1.OrderStatus.ACCEPTED) {
        ...
        const feedback = await prisma_1.prisma.orderFeedback.create({
            data: {
                order_id: orderId,
...
const getFarmerFeedbacks = async (req, res) => {
    try {
        const farmerId = req.user?.userId;
        if (!farmerId)
            return res.status(401).json({ success: false, message: 'Unauthorized' });
        const feedbacks = await prisma_1.prisma.orderFeedback.findMany({
...
exports.getFarmerFeedbacks = getFarmerFeedbacks;
