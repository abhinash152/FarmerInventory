...
exports.updateTrackingStep = exports.respondToOrder = exports.getCustomerOrders = exports.getFarmerOrders = exports.placeOrder = void 0;
...
const placeOrder = async (req, res) => {
    try {
        ...
        const { product_id, quantity, payment_method, delivery_pincode, delivery_address, } = req.body;
        ...
        const selectedPaymentMethod = payment_method === 'ONLINE' ? prisma_1.PaymentMethod.ONLINE : prisma_1.PaymentMethod.COD;
        const initialPaymentStatus = selectedPaymentMethod === 'ONLINE' ? prisma_1.PaymentStatus.COMPLETED : prisma_1.PaymentStatus.PENDING;
...
const respondToOrder = async (req, res) => {
    try {
        ...
        if (order.status !== prisma_1.OrderStatus.PENDING) {
            return res.status(400).json({
                success: false,
        ...
        const result = await prisma_1.prisma.$transaction(async (tx) => {
            ...
            if (action === 'ACCEPTED') {
                ...
                if (freshProduct.stock_quantity < order.quantity) {
                    throw new Error(`Insufficient stock to accept order! Available: ${freshProduct.stock_quantity} ${freshProduct.unit}, requested: ${order.quantity}`);
                }
                ...
                await tx.orderExtension.upsert({
                    where: { order_id: orderId },
                    update: {
                        tracking_step: prisma_1.TrackingStep.ACCEPTED,
                    },
                    create: {
                        order_id: orderId,
                        tracking_step: prisma_1.TrackingStep.ACCEPTED,
                ...
                const updatedOrder = await tx.order.update({
                    where: { order_id: orderId },
                    data: {
                        status: prisma_1.OrderStatus.REJECTED,
                        responded_at: now,
                    },
                });
...
const updateTrackingStep = async (req, res) => {
    try {
        const farmerId = req.user?.userId;
        const orderId = Number(req.params.id);
        ...
        const updatedExt = await prisma_1.prisma.orderExtension.upsert({
            where: { order_id: orderId },
            update: {
                tracking_step: tracking_step,
                ...(tracking_step === 'DELIVERED' ? { payment_status: prisma_1.PaymentStatus.COMPLETED } : {}),
            },
            create: {
                order_id: orderId,
                tracking_step: tracking_step,
...
exports.updateTrackingStep = updateTrackingStep;
