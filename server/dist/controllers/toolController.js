...
exports.calculatePincodeDelivery = exports.getMandiBenchmarks = void 0;
...
const getMandiBenchmarks = async (req, res) => {
    try {
        ...
        if (state && typeof state === 'string' && state.trim() !== '' && state !== 'All India' && state !== 'ALL') {
            ...
            const stateFiltered = results.filter((b) => b.state.toLowerCase().includes(targetState) || targetState.includes(b.state.toLowerCase()));
        ...
        if (crop && typeof crop === 'string') {
            ...
            const cropFiltered = results.filter((b) => b.crop_name.toLowerCase().includes(q) ||
            ...
            if (cropFiltered.length > 0) {
                ...
                const globalCropFiltered = MANDI_BENCHMARKS.filter((b) => b.crop_name.toLowerCase().includes(q) ||
                    b.hindi_name.toLowerCase().includes(q) ||
                    b.category.toLowerCase().includes(q));
        ...
        if (farmer_price && results.length > 0) {
            ...
            if (!isNaN(fPrice) && fPrice > 0) {
                const farmerProfitVsMandiPercent = Number((((fPrice - matched.apmc_mandi_price) / matched.apmc_mandi_price) * 100).toFixed(1));
                const customerSavingsVsRetailPercent = Number((((matched.retail_supermarket_price - fPrice) / matched.retail_supermarket_price) * 100).toFixed(1));
                customComparison = {
...
exports.getMandiBenchmarks = getMandiBenchmarks;
const calculatePincodeDelivery = async (req, res) => {
    try {
...
exports.calculatePincodeDelivery = calculatePincodeDelivery;
