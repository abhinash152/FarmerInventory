    ...
    if (role === 'FARMER') {
      ...
      const conversations = customerIds.map((cId) => {
        ...
        const unreadCount = cMessages.filter(
          (m) => m.sender_role === UserRole.CUSTOMER && !m.is_read
        ).length;
      ...
      const conversations = farmerIds.map((fId) => {
        ...
        const unreadCount = fMessages.filter(
          (m) => m.sender_role === UserRole.FARMER && !m.is_read
        ).length;
...
export const getMessages = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const opposingRole = role === 'FARMER' ? UserRole.CUSTOMER : UserRole.FARMER;
  ...
  try {
    ...
    const newMessage = await prisma.chatMessage.create({
      data: {
        ...
        sender_role: role === 'FARMER' ? UserRole.FARMER : UserRole.CUSTOMER,
