...
const CROP_DICTIONARY: Record<string, { standardName: string; category: string; defaultUnit: string; defaultPrice: number }> = {
 ...
 */
function parseVoiceAction(prompt: string, role: string) {
  ...
  if (isAddIntent || /\d+\s*(kg|किलो|quntal|quintal|लीटर|liter)/i.test(lower)) {
    ...
    // Find unit
    let unit = 'kg';
    if (lower.includes('quntal') || lower.includes('quintal') || lower.includes('क्विंटल')) unit = 'quintal';
    else if (lower.includes('liter') || lower.includes('litre') || lower.includes('लीटर') || lower.includes('ltr')) unit = 'liter';
    else if (lower.includes('dozen') || lower.includes('दर्जन')) unit = 'dozen';
    else if (lower.includes('packet') || lower.includes('पैकेट')) unit = 'packet';
