export interface FreshnessCalculation {
  score: number; // 0 - 100%
  rating10: number; // 0.0 - 10.0
  rating5: number; // 0.0 - 5.0
  tier: string;
  tierCode: 'PEAK' | 'VERY_FRESH' | 'GOOD' | 'MATURING' | 'AGING';
  colorHex: string;
  badgeBg: string;
  badgeText: string;
  badgeBorder: string;
  ...
  harvestDateFormatted: string;
}

export function calculateFreshness(
  harvestDateInput?: Date | string | null,
  productName: string = '',
  category: string = 'General',
  storageCondition: string = 'FIELD_FRESH',
  farmLocation: string = ''
): FreshnessCalculation {
  const now = new Date();
  const harvestDate = harvestDateInput ? new Date(harvestDateInput) : new Date(now.getTime() - 1000 * 60 * 60 * 18);
  ...
  // Determine crop profile: { baseShelfLifeDays, dailyDecayAmbient }
  let baseShelfLife = 10;
  let decayRate = 8.0;

  if (
    catLower.includes('grain') ||
  ...
  let tierCode: 'PEAK' | 'VERY_FRESH' | 'GOOD' | 'MATURING' | 'AGING' = 'PEAK';
